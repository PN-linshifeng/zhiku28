...
function getNewsList(cPage, ln, cat, num, onDate, endDate, keywords, isSearch) {
	...
	$.post(api, {
		...
		$.each(json.data, function(i, val) {
				...
				list += '<li><a href="news-content.html?aid=' + val.article_id + '" target="_blank" rel="' + val.article_id + '" title="' + val.title + '"><span class="time">' + val.add_time.substring(0, 10) + '</span>' + valTitle + '</a></li>';
			})
			// $('#main > div.news > ul').find('div.replaceImg').remove();
			// if (isSearch) {
			// 	$('#main > div.news > ul').html('');
			// }
		$('.news-list > ul').append(list).show();
...
function getMarketCommentaryList(cPage, ln, cat, num, onDate, endDate, keywords, isSearch) {
	...
	$.post(api, {
		...
		$.each(json.data, function(i, val) {
			list += '<li><a href="' + downloadUrl + val.file_url + '" target="_blank" rel="' + val.article_id + '"><span class="time">' + val.add_time.substring(0, 10) + '</span>' + val.title + '</a></li>';
		});
		// $('#main > div.news > ul').find('div.replaceImg').remove();
		$('.market-commentary-list > ul').append(list);
	}, 'json');
}
	...
	$.post(api, {
		...
		$.each(json.data, function(i, val) {
			if (i == 0) {
				...
				ddContent = '<a href="news-content.html?aid=' + val.article_id + '" target="_blank" rel="' + val.article_id + '">' + valContent + "</a>";
			} else {
				var valTitle = "";
				...
				list += '<li><a href="news-content.html?aid=' + val.article_id + '" target="_blank" rel="' + val.article_id + '"><span class="time">' + val.add_time + '</span>' + valTitle + '</a></li>';
			}
		})
...
function getNewsContentById(aid, ln) {
	...
	$.post(api, {
		...
		if (!!jsonContent.title) {
			if (jsonContent.title) {
				$('.news-title').html(jsonContent.title);
				document.title = jsonContent.title;
				// titleReplace(jsonContent.title);
			}
			if (jsonContent.content)
				$('.news-content').html(jsonContent.content);
			if (jsonContent.add_time) {
				$('.news-time').html(jsonContent.add_time);
				$('.source').html(jsonContent.source);
			}
 ...
 * 
 */
function getNewsPreNext(aid, ln) {
	var api = SystemProp.contentServiceUrl + '/getNewsList.php?product=forex&jsoncallback=?';
	$.post(api, {
		cat: 33,
		ln: ln || 'zh-cn',
		isTc: false,
		num: 2,
		startAt: Math.floor(Math.random() * 100)
	}, function(jsonContent) {
		if (jsonContent.msg === 'ok') {
			var html = '<div class="f-l">上一篇 <a href="news-content.html?aid=' + jsonContent.data[0].article_id + '" title="' + jsonContent.data[0].title + '">' + jsonContent.data[0].title + '</a></div><div class="f-r">下一篇 <a href="news-content.html?aid=' + jsonContent.data[1].article_id + '" title="' + jsonContent.data[1].title + '">' + jsonContent.data[1].title + '</a></div>';
			$(".pre-next").html(html)
		}
	}, 'json')
}
...
$(document).ready(function(e) {
	...
	getMarketCommentaryList(1, 'zh-cn', 38, '20', '', '', '', false);
